...
# Initialize Azure OpenAI client with settings from environment variables
client = AzureOpenAI(
...
# Get deployment name from environment variable or use default
...
def gather_scraped_content(url: Optional[str] = None, max_pages: Optional[int] = 3, force_crawl: bool = False) -> str:
    ...
    if url:
        ...
        if force_crawl or not os.path.exists(filename):
            ...
            try:
                scraped_filename, _ = crawl_website(url, page_limit=max_pages if max_pages is not None else 3)
    ...
    try:
        pages_content = load_scraped_website_content(filename=filename, max_pages=max_pages if max_pages is not None else 3)
    ...
    while attempt < max_retries:
        try:
            ...
            if raw_json_output is None:
                raise ValueError("OpenAI returned None as response content")
